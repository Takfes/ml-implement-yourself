...
class DTClassifier:
    ...
    def fit(self,X,y):
        self.n_feats = X.shape[1] if not self.n_feats else max(self.n_feats,X.shape[1])
    ...
    def _grow_tree(self,X,y, depth=0):
        ...
        # stopping criteria 
        if (n_samples <= self.min_samples or 
        ...
        # greedy search
        best_feat, best_thres = self._best_criteria(X,y,feats_idxs)
    ...
    def _information_gain(self,y,X_column,split_thres):
        # parent entropy
        parent_etnropy = self._entropy(y)
        # generate split
        ...
        # return ig
        ig = parent_etnropy - child_entropy
        return ig
